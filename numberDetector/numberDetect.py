...
def activateCam():
    ...
    while cam.isOpened():
        ...
        if (current_time > 1./ FPS) : 
            ...
            carNum = textRead(process_image)

            cv2.rectangle(img_copy, (x+startX, y+startY), (x+endX, y+endY), (0, 255, 0), 2)

            cv2.putText(img_copy, carNum, (x+startX, y+startY-10),
                cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)

            # show the output image
            cv2.imshow("OCR Text Recognition : "+carNum, img_copy)
            #######################

            if status:
                cv2.imshow("VideoCapture", 255 - img)
                pass
